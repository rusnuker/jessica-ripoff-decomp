...
public final class WorldEntitySpawner {
    ...
    public int findChunksForSpawning(WorldServer worldServerIn, boolean spawnHostileMobs, boolean spawnPeacefulMobs, boolean spawnOnSetTickRate) {
            ...
            while (i1 <= 8) {
                ...
                while (j1 <= 8) {
                    ...
                    if (!this.eligibleChunksForSpawning.contains(chunkpos)) {
                        ...
                        ++i;
                        if (!flag && worldServerIn.getWorldBorder().contains(chunkpos) && (playerchunkmapentry = worldServerIn.getPlayerChunkMap().getEntry(chunkpos.chunkXPos, chunkpos.chunkZPos)) != null && playerchunkmapentry.isSentToPlayers()) {
        ...
        while (n2 < n) {
            ...
            if (!(enumcreaturetype.getPeacefulCreature() && !spawnPeacefulMobs || !enumcreaturetype.getPeacefulCreature() && !spawnHostileMobs || enumcreaturetype.getAnimal() && !spawnOnSetTickRate || (k4 = worldServerIn.countEntities(enumcreaturetype.getCreatureClass())) > (l4 = enumcreaturetype.getMaxNumberOfCreature() * i / MOB_COUNT_DIV))) {
                    ...
                    while (k2 < 3) {
                        ...
                        while (i4 < l3) {
                            ...
                            if (!worldServerIn.isAnyPlayerWithinRangeAt(f, i3, f1, 24.0) && blockpos1.distanceSq(f, i3, f1) >= 576.0) {
                                if (biome$spawnlistentry == null && (biome$spawnlistentry = worldServerIn.getSpawnListEntryForTypeAt(enumcreaturetype, blockpos$mutableblockpos)) == null) break;
                                if (worldServerIn.canCreatureTypeSpawnHere(enumcreaturetype, biome$spawnlistentry, blockpos$mutableblockpos) && WorldEntitySpawner.canCreatureTypeSpawnAtLocation(EntitySpawnPlacementRegistry.getPlacementForEntity(biome$spawnlistentry.entityClass), worldServerIn, blockpos$mutableblockpos)) {
