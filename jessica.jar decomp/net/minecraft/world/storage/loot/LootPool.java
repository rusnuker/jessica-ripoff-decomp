...
public class LootPool {
    ...
    protected void createLootRoll(Collection<ItemStack> stacks, Random rand, LootContext context) {
        ...
        int i = 0;
        LootEntry[] lootEntryArray = this.lootEntries;
        int n = this.lootEntries.length;
        int n2 = 0;
        while (n2 < n) {
            int j;
            LootEntry lootentry = lootEntryArray[n2];
            if (LootConditionManager.testAllConditions(lootentry.conditions, rand, context) && (j = lootentry.getEffectiveWeight(context.getLuck())) > 0) {
                list.add(lootentry);
                i += j;
            }
            ++n2;
