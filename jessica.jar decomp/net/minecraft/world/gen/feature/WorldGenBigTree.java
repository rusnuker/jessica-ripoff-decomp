    ...
    void crosSection(BlockPos pos, float p_181631_2_, IBlockState p_181631_3_) {
        ...
        while (j <= i) {
            ...
            while (k <= i) {
                BlockPos blockpos;
                Material material;
                if (Math.pow((double)Math.abs(j) + 0.5, 2.0) + Math.pow((double)Math.abs(k) + 0.5, 2.0) <= (double)(p_181631_2_ * p_181631_2_) && ((material = this.world.getBlockState(blockpos = pos.add(j, 0, k)).getMaterial()) == Material.AIR || material == Material.LEAVES)) {
