...
public class PlayerControllerMP {
    ...
    public void attackEntity(EntityPlayer playerIn, Entity targetEntity) {
        try {
            if (Wrapper.getModule("Reach").isToggled() && (double)Wrapper.player().getDistanceToEntity(targetEntity) > Reach_One_Million_Dollars.reach) {
                return;
            }
            this.syncCurrentPlayItem();
            if (Wrapper.mc().gameSettings.keyBindUseItem.pressed && Wrapper.getModule("AutoBlock").isToggled() && Wrapper.player().getHeldItem(EnumHand.MAIN_HAND).getItem() != null && Wrapper.player().getHeldItem(EnumHand.MAIN_HAND).getItem() instanceof ItemSword) {
