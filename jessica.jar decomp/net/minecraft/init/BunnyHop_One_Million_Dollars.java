...
public class BunnyHop_One_Million_Dollars
extends Module {
    ...
    public BunnyHop_One_Million_Dollars() {
        super("BunnyHop", Category.Player);
    }

    @Override
    @EventTarget
    public void onMotion(MoveEvent event) {
        if (Wrapper.mc().gameSettings.keyBindJump.pressed && Wrapper.player().hurtTime == 0) {
            this.jump = true;
            ...
            if (EntityUtils.round(Wrapper.player().posY - (double)((int)Wrapper.player().posY), 3) == EntityUtils.round(0.138, 3)) {
            ...
            if (this.level == 1 && (Wrapper.player().moveForward != 0.0f || Wrapper.player().moveStrafing != 0.0f)) {
                ...
                if (this.level == 3) {
                    ...
                    this.moveSpeed = (this.lastDist - difference) * 1.0;
                } else {
