...
public abstract class PlayerList {
    ...
    public String getFormattedListOfPlayers(boolean includeUUIDs) {
        ...
        while (i < list.size()) {
            ...
            if (includeUUIDs) {
                s = String.valueOf(s) + " (" + ((EntityPlayerMP)list.get(i)).getCachedUniqueIdString() + ")";
            }
            ++i;
        }
        return s;
    ...
    public void sendToAllNearExcept(@Nullable EntityPlayer except, double x, double y, double z, double radius, int dimension, Packet<?> packetIn) {
        ...
        while (i < this.playerEntityList.size()) {
            ...
            double d0;
            EntityPlayerMP entityplayermp = this.playerEntityList.get(i);
            if (entityplayermp != except && entityplayermp.dimension == dimension && (d0 = x - entityplayermp.posX) * d0 + (d1 = y - entityplayermp.posY) * d1 + (d2 = z - entityplayermp.posZ) * d2 < radius * radius) {
                entityplayermp.connection.sendPacket(packetIn);
            }
            ++i;
        }
