...
extends CommandBase {
    ...
    public List<String> getTabCompletionOptions(MinecraftServer server, ICommandSender sender, String[] args, @Nullable BlockPos pos) {
        if (args.length == 1) {
            ...
            GameProfile[] gameProfileArray = server.getGameProfiles();
            int n = gameProfileArray.length;
            int n2 = 0;
            while (n2 < n) {
                GameProfile gameprofile = gameProfileArray[n2];
                if (!server.getPlayerList().canSendCommands(gameprofile) && CommandOp.doesStringStartWith(s, gameprofile.getName())) {
