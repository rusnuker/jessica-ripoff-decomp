...
public class TileSet {
    ...
    public TileSet(TiledMap map, Element element, boolean loadImage) throws SlickException {
        ...
        if (source != null && !source.equals("")) {
            try {
                Element docElement;
                InputStream in = ResourceLoader.getResourceAsStream(String.valueOf(map.getTilesLocation()) + "/" + source);
        ...
        if (loadImage) {
            Image image = new Image(String.valueOf(map.getTilesLocation()) + "/" + ref, false, 2, trans);
            this.setTileSetImage(image);
        }
        NodeList pElements = element.getElementsByTagName("tile");
        int i = 0;
        while (i < pElements.getLength()) {
