...
public class ObjectTreeParser {
    ...
    private Object traverse(XMLElement current, Object instance) throws SlickXMLException {
        ...
        try {
            ...
            while (i2 < children.size()) {
                XMLElement element = children.get(i2);
    ...
    private Method findMethod(Class clazz, String name) {
        ...
        while (i < methods.length) {
            Method method;
            Class<?>[] params;
            if (methods[i].getName().equalsIgnoreCase(name) && (params = (method = methods[i]).getParameterTypes()).length == 1) {
                return method;
            }
            ++i;
        }
    ...
    private Method findMethod(Class clazz, String name, Class parameter) {
        ...
        while (i < methods.length) {
            Method method;
            Class<?>[] params;
            if (methods[i].getName().equalsIgnoreCase(name) && (params = (method = methods[i]).getParameterTypes()).length == 1 && method.getParameterTypes()[0].isAssignableFrom(parameter)) {
                return method;
            }
            ++i;
        }
