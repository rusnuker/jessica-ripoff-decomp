...
public class Input {
    ...
    public void poll(int width, int height) {
        ...
        block5: while (Keyboard.next()) {
            ...
            if (Keyboard.getEventKeyState()) {
                ...
                this.consumed = false;
                i2 = 0;
                while (i2 < this.keyListeners.size()) {
        ...
        block8: while (Mouse.next()) {
            if (Mouse.getEventButton() >= 0) {
                if (Mouse.getEventButtonState()) {
                    ...
                    int i3 = 0;
                    while (i3 < this.mouseListeners.size()) {
                ...
                int i4 = 0;
                while (i4 < this.mouseListeners.size()) {
            ...
            if (Mouse.isGrabbed() && this.displayActive && (Mouse.getEventDX() != 0 || Mouse.getEventDY() != 0)) {
                this.consumed = false;
                int i5 = 0;
                while (i5 < this.mouseListeners.size()) {
            ...
            this.consumed = false;
            int i6 = 0;
            while (i6 < this.mouseListeners.size()) {
        ...
        if (!this.displayActive || Mouse.isGrabbed()) {
            ...
            this.consumed = false;
            int i7 = 0;
            while (i7 < this.mouseListeners.size()) {
        ...
        if (controllersInited) {
            int i8 = 0;
            while (i8 < this.getControllerCount()) {
                int count = ((Controller)controllers.get(i8)).getButtonCount() + 3;
                count = Math.min(count, 24);
                int c = 0;
                while (c <= count) {
                    if (this.controls[i8][c] && !this.isControlDwn(c, i8)) {
                        this.controls[i8][c] = false;
                        this.fireControlRelease(c, i8);
                    } else if (!this.controls[i8][c] && this.isControlDwn(c, i8)) {
                        this.controllerPressed[i8][c] = true;
                        this.controls[i8][c] = true;
                        this.fireControlPress(c, i8);
                    }
                    ++c;
                }
                ++i8;
        ...
        if (this.keyRepeat) {
            int i9 = 0;
            while (i9 < 1024) {
                if (this.pressed[i9] && this.nextRepeat[i9] != 0L && System.currentTimeMillis() > this.nextRepeat[i9]) {
                    this.nextRepeat[i9] = System.currentTimeMillis() + (long)this.keyRepeatInterval;
                    this.consumed = false;
                    int j = 0;
                    while (j < this.keyListeners.size()) {
    ...
    private void fireControlPress(int index, int controllerIndex) {
        this.consumed = false;
        int i = 0;
        while (i < this.controllerListeners.size()) {
    ...
    private void fireMouseClicked(int button, int x, int y, int clickCount) {
        this.consumed = false;
        int i = 0;
        while (i < this.mouseListeners.size()) {
